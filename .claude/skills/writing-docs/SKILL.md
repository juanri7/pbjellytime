...
```md
<Experimental badge>This feature is experimental.</Experimental>
```
...
**For page-level version indicators**, use an `# h1` heading with `<AvailableFrom version="4.0" />` inline so it appears next to the title (not below it). Use `&lt;` and `&gt;` to escape angle brackets in component names:

```md
# &lt;MyComponent&gt; <AvailableFrom version="4.0" />
```

```md
# @remotion/my-package <AvailableFrom version="4.0" />
```

For section headings:

```md
## Saving to another cloud <AvailableFrom version="4.0" />
```
...
```md
### onError? <AvailableFrom version="4.0" />
...
```md
### codec? <AvailableFrom version="4.0" />

Optional since 4.0. Previously required.
```
