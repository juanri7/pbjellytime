...
The type in `AnyRemotionOption` and `type: as T` determines the option's value type. Use `boolean`, `string | null`, `number | null`, etc.
...
- In the `CommandLineOptions` type, add: `[myFlagOption.cliFlag]: TypeOfOption;`
...
- Use `<OptionDescription name="myFlagOption" />` to pull the description from the option definition automatically
- The `id` matches the option's `cliFlag` value

**`packages/docs/docs/config.mdx`**:

- Add a `## \`setMyFlagEnabled()\`` section with:
  - `<OptionDescription name="myFlagOption" />` for the description
