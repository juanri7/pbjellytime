...
1. **Create `packages/<package-name>/`** with these files:
...
2. **Register in monorepo:**
   - `tsconfig.json` (root) — add `{"path": "./packages/<package-name>"}` to references
   - `packages/cli/src/list-of-remotion-packages.ts` — add `'@remotion/<package-name>'`
   - `packages/create-video/src/list-of-remotion-packages.ts` — add `'@remotion/<package-name>'`
   - `packages/studio-shared/src/package-info.ts` — add to `packages`, `descriptions`, `installableMap`, `apiDocs`

3. **Documentation (`packages/docs/docs/<package-name>/`):**
   - Add `"@remotion/<package-name>": "workspace:*"` to `packages/docs/package.json` dependencies (needed for twoslash snippets)
...
4. **Example usage:**
   - Add `"@remotion/<package-name>": "workspace:*"` to `packages/example/package.json`
   - Create `packages/example/src/<package-name>/index.tsx`
   - Register in `packages/example/src/Root.tsx`
   - Add `{"path": "../<package-name>"}` to `packages/example/tsconfig.json` references

5. **Run `bun i`** to install dependencies

6. **Build:** `cd packages/<package-name> && bun run make`
