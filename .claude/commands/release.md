...
- Run `npm token create --name="PublishRemotionXXXXXX" --packages "remotion" --packages "create-video" --packages-and-scopes-permission read-write --bypass-2fa --scopes "@remotion" --otp= `. Replace XXXXXX with a random string so we have a unique name. Use `op item get "Npmjs" --otp` to get the OTP and pass it via `--otp=`. It will ask for a password, pipe in the password using `<<<`. The NPM token will be printed.
- Run `bun i`
- Run `bun run build`
- Check `https://www.npmjs.com/package/remotion` to get the current version number
- Run `bun set-version.ts `, where is the current version plus 1
- Run `NPM_CONFIG_TOKEN= bun run release` where is the NPM token we just created
